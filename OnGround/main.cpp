#include <iostream>
#include <fstream>
#include <inttypes.h>
#include <vector>
#include <string>

using namespace std;

#pragma pack(push, 1)
typedef struct {
	uint32_t marker;
	uint32_t number;

	int32_t press_b;
	int32_t temp_b;
	int16_t temp_ds;
	uint16_t power_ina;
	int16_t lights[8];
	int16_t accelerations[3];
	uint8_t servo_pos[3];
	uint32_t time;

	int8_t e_bmp;
	int8_t e_ds;
	int8_t e_adxl;
	int8_t e_ads1;
	int8_t e_ads2;
	int8_t e_ina;
	int8_t e_sd;

	uint8_t checksum;
} tel_t;
#pragma pack(pop)
const uint8_t crc8_table[] = {
    
};

uint8_t crc8_update(uint8_t crc, uint8_t b) {
    return crc8_table[crc ^ b];
}


uint8_t crc8(const void * data_ptr, size_t data_size)
{
    const uint8_t * byte_ptr = (const uint8_t*)data_ptr;
    uint8_t crc_value = 0;
    for (size_t i = 0; i < data_size; i++)
    {
        crc_value = crc8_update(crc_value, *(byte_ptr+i));
    }

    return crc_value;
}


int main()
{
    ifstream iFile("telemetry.log", ios::binary);
    ofstream oFile("out.csv", ios::out | std::ios::trunc);

    int count = 0;

    tel_t packet;
    uint32_t Marker = 0xFAFCFAFD;

    iFile.seekg(0, std::ios::end);
    unsigned long filesize = iFile.tellg();
    iFile.seekg(0);
    unsigned long pos = 0;

    while(pos + sizeof(packet) < filesize)
    {
        uint32_t MarkerFinder;
        iFile.seekg(pos);
        iFile.read((char*)&MarkerFinder, sizeof(MarkerFinder));
        //cout<<"MARKER FINDER IS "<<hex<<MarkerFinder<<dec<<endl;
        if(MarkerFinder == Marker){
        	iFile.seekg(pos);
            cout<<"MARKER OCCURED: TRYING TO READ PACKET..."<<endl;
            iFile.read((char*)&packet, sizeof(packet));
            cout<<"PACKET READED, CHECKING HASH..."<<endl;
            if(crc8(&packet, sizeof(packet) - sizeof(packet.checksum)) == packet.checksum)
            {
            	count++;
                cout<<"HASH RIGHT! WRITING PACKET WITH NUM "<<packet.number<<" ON POS "<<pos<<endl;
                pos += sizeof(packet);
                oFile<<packet.marker<<';'<<packet.number<<';'<<packet.press_b<<';'<<packet.temp_b<<';'
                		<<packet.temp_ds<<';'<<packet.power_ina<<';';
                for(int i = 0; i < 8; i++)
                	oFile<<packet.lights[i]<<';';
                for(int i = 0; i < 3; i++)
                	oFile<<packet.accelerations[i]<<';';
                for(int i = 0; i < 3; i++)
                    oFile<<(int)packet.servo_pos[i]<<';';

                oFile<<(int)packet.time<<';'<<(int)packet.e_bmp<<';'<<(int)packet.e_ds<<';'
                		<<(int)packet.e_adxl<<';'<<(int)packet.e_ads1<<';'
                		<<(int)packet.e_ads2<<';'<<(int)packet.e_ina<<';'
                		<<(int)packet.e_sd<<endl;
            }
            else
            {
                cout<<"HASH WRONG! CONTINUE..."<<endl;
                pos++;
            }
        }
        else{
            pos++;
        }

    }
    cout<<"FINDED "<<count<<" PACKETS, NUMBER OF THE LAST PACKET IS "<<packet.number<<endl;
    iFile.close();
    oFile.close();
}
